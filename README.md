...
同一上下文**连续完成 6 次刷新后自动暂停**。点击「恢复」可以开始新一轮保活；新的正常生成也会替换请求快照并重新开始计数。请求出错或超过 180 秒未完成时会暂停，不会循环自动重试。
...
开启“测试：回复结束后开始计时”，沿用原间隔（通常 4 分钟），从正常回复结束开始等待；后续保活也从回复结束计时。默认仍从请求发出计时。切换模式后需重新正常发送一次；测试模式点击恢复也会等待新的正常请求。此模式用于延后刷新、观察缓存有效期，单次命中不能直接证明服务商的缓存时长或起算点。
