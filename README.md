...
**间隔从请求发出时计算，包含模型输出耗时。** 例如 00:00 发出正常请求，02:00 回复结束，首次保活约在 04:00 发送，而不是 06:00。后续保活也按上一次保活的发出时间计算期限。正常生成期间不并发保活；如果回复结束时已经到期，就在下一次检查时发送一次，不补发多次。超过 10 分钟的极端输出不作额外保活处理，期间缓存可能过期。手动点击「恢复」则从点击时重新等待一个间隔。
