...
切换对话、编辑历史、切换模型、预设或连接，以及修改世界信息，都会使已保存的请求失效。此时需要正常发送一次消息，建立新的请求快照。生成期间比较已发送的输入部分及相关设置，排除正在写入的回复；继续生成或切换候选回复时，也会排除正在改写的输出位置。历史编辑和模型、连接等变更仍会使旧快照失效。正常回复结束后恢复完整聊天记录、元数据、当前角色或群组、提示词设置及扩展设置的比较。辅助性的静默请求不参与保活。请求正文不会写入设置或浏览器存储。
